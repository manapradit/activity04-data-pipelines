...
**Response**: `message = FALSE` only shows the code chunk without
messages from R. `message = TRUE` shows long messages from R.
...
**Response**: It shows column names.
...
**Response**: I will use major and unemployment\_rate variable.
...
``` r
arrange(college_recent_grads,sharewomen) %>% 
...
**Response**: Military Technologies major has no woman in the major.
